export const TASK_RECT_WIDTH = 200;
export const TASK_RECT_HEIGHT = 100;
export const TASK_RECT_HEIGHT_HALF = TASK_RECT_HEIGHT / 2;
export const TASK_HANDLE_SIZE = 16;
export const TASK_HANDLE_SIZE_HALF = TASK_HANDLE_SIZE / 2;
export const TASK_HANDLE_Y_SHIFT = TASK_RECT_HEIGHT_HALF - TASK_HANDLE_SIZE_HALF;
export const TASK_MENU_BUTTON_SIZE = 24;
export const TASK_MENU_BUTTON_PADDING = 10;
export const EDGE_HANDLE_SIZE = 16;
export const EDGE_HANDLE_SIZE_HALF = EDGE_HANDLE_SIZE / 2;
export const AXIS_OFFSET = 100;
export const AXIS_INSERT_BUTTON_CLASS = "cs-insert-axis";
export const AXIS_INSERT_BUTTON_SELECTOR = `.${AXIS_INSERT_BUTTON_CLASS}`;
export const AXIS_UPDATE_BUTTON_CLASS = "cs-update-axis";
export const AXIS_UPDATE_BUTTON_SELECTOR = `.${AXIS_UPDATE_BUTTON_CLASS}`;
export const AXIS_DELETE_BUTTON_CLASS = "cs-delete-axis";
export const AXIS_DELETE_BUTTON_SELECTOR = `.${AXIS_DELETE_BUTTON_CLASS}`;
export const SVG_CLASS = "cs-svg-select";
export const SVG_SELECTOR = `.${SVG_CLASS}`;
export const SELECTABLE_GROUP_CLASS = "cs-selectable";
export const SELECTABLE_GROUP_SELECTOR = `.${SELECTABLE_GROUP_CLASS}`;
export const TASK_UPDATE_BUTTON_CLASS = "cs-update-task";
export const TASK_UPDATE_BUTTON_SELECTOR = `.${TASK_UPDATE_BUTTON_CLASS}`;
